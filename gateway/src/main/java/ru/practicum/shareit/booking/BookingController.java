...
public class BookingController {
    ...
    public ResponseEntity<Object> changeApproveBooking(
            @PathVariable Integer bookingId,
        ...
        return bookingClient.changeApproveBooking(bookingId, approverUserId, approved);
    }

    @GetMapping("/owner")
    public ResponseEntity<Object> getAllOwnerBookingRequestsInState(
            @RequestParam(name = "state", defaultValue = "ALL") String stateParam,
            @RequestParam(value = "from", defaultValue = "0") @PositiveOrZero Integer from,
            @RequestParam(value = "size", defaultValue = "10") Integer size,
            @RequestHeader("X-Sharer-User-Id") Integer ownerId
    ) {
        log.info("Get request to get all owner's bookings info received.");
        BookingState state = BookingState.from(stateParam)
                .orElseThrow(() -> new IllegalArgumentException("Unknown state: " + stateParam));
        return bookingClient.findAllOwnerBookingsWithState(
                ownerId,
                state,
                from,
                size
